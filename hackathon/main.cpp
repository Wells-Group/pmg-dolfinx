...
int main(int argc, char* argv[])
{
  ...
  PetscInitialize(&argc, &argv, nullptr, nullptr);
  {
    ...
    // std::cout << acc::norm(x, dolfinx::la::Norm::l2) << std::endl;

    // Mat A = op.host_matrix();
    ...
    // Vec _b = la::petsc::create_vector_wrap(*V->dofmap()->index_map,
    // V->dofmap()->index_map_bs(), b.mutable_array()); Vec _x; VecDuplicate(_b,
    // &_x);
    ...
    // PetscDeviceType device_type = PETSC_DEVICE_DEFAULT();
    // PetscDevice device;
    // PetscDeviceCreate(device_type, 0, &device);
    // PetscDeviceView(device, PETSC_VIEWER_STDOUT_WORLD);
